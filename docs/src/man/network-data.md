...
Both parameters `status_p` and `status_n` are used for a bipolar converter (i.e.,
`poles = 2`).
Instead, a single status parameter is selected if the converter is monopolar (i.e.,
`poles = 1`), depending on its specific configuration:

- In **symmetric** configuration (i.e., `connect_at = 0`), the `status_p` parameter is used
  by default.
- In **asymmetric** configuration (i.e., `connect_at` set at `1` or `2`), the status
  parameter is chosen according to the positive/negative DC bus terminal to which the
  converter is connected.
  For example, parameter `status_p` is used if the monopolar converter is connected to the
