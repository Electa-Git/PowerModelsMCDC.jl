...
function build_mc_data!(base_data)
 ...
 #changing the connection point
    ...
    for (c,bn) in mp_data["branchdc"]
        ...
        gama = 0.1 # scalling factor for power limit of metalic return
        bn["rateA"][metalic_cond_number]=bn["rateA"][metalic_cond_number]*gama
        bn["rateB"][metalic_cond_number]=bn["rateB"][metalic_cond_number]*gama
        bn["rateC"][metalic_cond_number]=bn["rateC"][metalic_cond_number]*gama
...
for i in 1:3
     ...
     display("power Idc at converter $i")
    display(result_mcdc["solution"]["convdc"]["$i"]["iconv_dc"])
end
for i in 1:3
     # display("power from grid to dc at converter $i")
     display("power pconv at converter $i")
    display(result_mcdc["solution"]["convdc"]["$i"]["pconv"])
end

for i in 1:3
    display("flow of over dc branch $i")
    display(result_acdc["solution"]["branchdc"]["$i"])
end
