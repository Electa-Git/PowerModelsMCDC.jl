...
function sol_component_value_edge_status(pm::_PM.AbstractPowerModel, n::Int, comp_name::Symbol, field_name_fr::Symbol, field_name_to::Symbol, comp_ids_fr, comp_ids_to, conductors, variables, constant=0.0)
    ...
    for (l, i, j) in comp_ids_fr
        data[(l, i, j)] = [
    haskey(variables[(l,i,j)], c) ? variables[(l,i,j)][c] : constant
    for c in conductors[l]]
    end
...
function sol_component_value_edge_status_new(
    ...
    comp_ids_fr,
    comp_ids_to,
    conductors::Dict{Int,Any},   # map: l -> iterable of conductors
    variables,
    constant=0.0
)
    data = Dict{Tuple{Int, Int, Int}, Any}()
    ...
    _PM.sol_component_value_edge(
        pm, n, comp_name, field_name_fr, field_name_to,
        comp_ids_fr, comp_ids_to, data
    )
end
...
function sol_component_value_status_new(pm::_PM.AbstractPowerModel, n::Int, comp_name::Symbol, field_name::Symbol, comp_ids, conductors, variables, constant=0.0)
...
"builds solution structure composed by both edge variables and fixed values based on multi-conductor status parameter"
function sol_component_value_edge_status_new(pm::_PM.AbstractPowerModel, n::Int, comp_name::Symbol, field_name_fr::Symbol, field_name_to::Symbol, comp_ids_fr, comp_ids_to, conductors, variables, constant=0.0)
