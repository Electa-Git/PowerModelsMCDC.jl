...
function add_ref_dcgrid_dcswitch!(ref::Dict{Symbol,<:Any}, nw_ref::Dict{String,<:Any})
    for (n, nw_ref) in ref[:it][_PM.pm_it_sym][:nw]
        ...
        # DC arcs: tuples of the form (l,i,j) where l is the DC branch and i and j are the adjacent DC buses
        nw_ref[:arcsdc_from] = [(b, branch["fbusdc"][cond], branch["tbusdc"][cond],cond) for (b, branch) in nw_ref[:branchdc] for cond in keys(nw_ref[:branchdc][b]["status"])]
        nw_ref[:arcsdc_to]   = [(b, branch["tbusdc"][cond], branch["fbusdc"][cond],cond) for (b, branch) in nw_ref[:branchdc] for cond in keys(nw_ref[:branchdc][b]["status"])]
        nw_ref[:arcsdc]      = [nw_ref[:arcsdc_from]; nw_ref[:arcsdc_to]]
        ...
        println("branchdc_conductors: ", nw_ref[:branchdc_conductors])
        ...
        # Switch part
        nw_ref[:arcs_from_dcsw] = [(i,switch["f_busdc"],switch["t_busdc"],terminal) for (i,switch) in nw_ref[:dcswitch] for terminal in nw_ref[:dcswitch][i]["terminal"]]
        nw_ref[:arcs_to_dcsw]   = [(i,switch["t_busdc"],switch["f_busdc"],terminal) for (i,switch) in nw_ref[:dcswitch] for terminal in nw_ref[:dcswitch][i]["terminal"]]
        nw_ref[:arcs_dcsw] = [nw_ref[:arcs_from_dcsw]; nw_ref[:arcs_to_dcsw]]

        bus_arcs_dcsw = Dict((i, Tuple{Int,Int,Int,String}[]) for (i,bus) in nw_ref[:busdc])
        for (l,i,j,terminal) in nw_ref[:arcs_dcsw]
            println("using l,i,j,terminal: ", (l,i,j,"$terminal"))
            push!(bus_arcs_dcsw[i], (l,i,j,"$terminal"))
        end
        nw_ref[:bus_arcs_dcsw] = bus_arcs_dcsw
        ...
        for (l,i,j,conductor) in nw_ref[:arcsdc]
            println("using l,i,j,conductor: ", (l,i,j,conductor))
            for terminal in nw_ref[:branchdc_conductors][l]
                println("using terminal: ", terminal)
                if terminal == conductor
                    push!(busdc_terminal_arcsdc[i,terminal], (l,i,j,conductor))
                end
            end
        end
        nw_ref[:busdc_terminal_arcsdc] = busdc_terminal_arcsdc
        ...
        for (c,conv) in nw_ref[:convdc]
            if conv["ground_type"] == 1
                poles = eachindex(conv["Pacrated"])
                push!(busdc_grounded_convs[conv["busdc_i"][first(poles)]], c)
            end
        end
        nw_ref[:busdc_grounded_convs] = busdc_grounded_convs
