...
function solve_mcdcpf(file::String, model_type::Type, solver; kwargs...)
    ...
    return solve_mcdcpf(data::Dict{String,Any}, model_type, solver; ref_extensions = [add_ref_dcgrid!], kwargs...)
end
...
function build_mcdcpf(pm::_PM.AbstractPowerModel)
   ...
   display(_PM.constraint_model_voltage(pm))
   display(constraint_voltage_dc(pm))

    for (i,bus) in _PM.ref(pm, :ref_buses)
        @assert bus["bus_type"] == 3
        display(_PM.constraint_theta_ref(pm, i))
        display(_PM.constraint_voltage_magnitude_setpoint(pm, i))
    end


    for (i, bus) in _PM.ref(pm, :bus)
        display(constraint_kcl_shunt(pm, i))
        # PV Bus Constraints
        if length(_PM.ref(pm, :bus_gens, i)) > 0 && !(i in _PM.ids(pm,:ref_buses))
            ...
            for j in _PM.ref(pm, :bus_gens, i)
                display("gen setpoint constraint for at bus $i")
                display(_PM.constraint_gen_setpoint_active(pm, j))
            end
        end
    end
    ...
    for (c, conv) in _PM.ref(pm, :convdc)
        ...
        if conv["type_dc"] == 2 "dc_type =2: Vdc control"
            ...
            elseif  conv["type_ac"] == 2
                constraint_dc_voltage_magnitude_setpoint(pm, c)
                # constraint_ac_voltage_magnitude_setpoint(pm, c)
            else
