...
""
function run_mcdctnepopf(file::String, model_type::Type, solver; kwargs...)
...
""
function run_mcdctnepopf(data::Dict{String,Any}, model_type::Type, solver; kwargs...)
    return _PM.run_model(data, model_type, solver, post_tnepopf; ref_extensions = [add_ref_dcgrid!, add_candidate_dcgrid!], kwargs...)
end

""
function post_mcdctnepopf(pm::_PM.AbstractPowerModel)
    ...
    variable_mc_dcbranch_current(pm)
    variable_mcdc_converter(pm)

    # new variables for TNEP problem
    variable_mc_active_dcbranch_flow_ne(pm)
    variable_branch_ne(pm)
    variable_mcdc_converter_ne(pm)# add more variables in variableconv.jl
    variable_mc_dcbranch_current_ne(pm)
    variable_mcdcgrid_voltage_magnitude_ne(pm)
